...
| 值 | 后端处理 |
|---|---|
| `pending` | 保存为草稿，通过 `draft_*` 返回，不覆盖正式数量 |
| `reviewed` | 人工节点成为正式方案，写入 `confirmed_*` 和 `effective_*` |
| `executed` | 数量口径与 reviewed 相同，额外表示业务已发货 |

复核仅在 `decision_signature` 与当前渠道、时效模式、计划日、旺季天数、停止收货日和预计入仓日一致时生效。
...
planned_ship_total
draft_scenario_nodes / draft_final_buy_qty
confirmed_scenario_nodes / confirmed_*_qty
effective_*_qty / effective_planned_ship_total
effective_next_buy_gap / effective_buy_qty
...
| 字段前缀 | 含义 |
|---|---|
| 无前缀的渠道数量 | 当前参数重新计算出的系统原始建议 |
| `confirmed_` | 已复核或已执行的人工正式数量 |
| `effective_` | 页面汇总、正式列表和 Excel 应使用的最终口径 |

待复核节点通过 `draft_scenario_nodes` 返回给前端恢复，但不会写入 `effective_`。这样既能保存草稿，又不会让未经确认的数量进入执行文件。

### 6.5 人工节点重算的数据流

```mermaid
flowchart LR
    A["前端修改节点数量"] --> B["数量自动标记为 quantity_locked"]
    B --> C["POST /api/scenario"]
    C --> D["recalculate_scenario_plan"]
    D --> E["生成现有日期及下一复核周期的渠道候选"]
    E --> F["_optimize_inventory_balance"]
    F --> G["固定人工量并逐日填补断货缺口"]
    G --> H["返回调整节点、断货状态和买货缺口"]
    H --> I["刷新橙线和节点"]
    I --> J["POST /api/decision 保存状态"]
```

`_optimize_inventory_balance` 使用统一库存守恒约束：
...
候选节点不是任意日期搜索。后端以当前计划日、用户已有节点日期及各日期加一个复核周期为发货日候选，再由 `build_channel_plans` 计算每条启用渠道的最终预计入仓日。
...
渠道建议列读取 `effective_{channel}_qty`。因此 pending 导出系统量，reviewed/executed 导出人工正式量。多个相同渠道的日期节点在保存时先汇总到 `confirmed_*_qty`，当前简表不会展开每个批次日期。
...
- 自动接力只在现有计划日和下一复核周期候选中搜索，不做任意日期或运输成本全局最优求解。
- reviewed 与 executed 当前使用相同数量公式；已执行节点尚未自动转成跨周未同步在途台账。
- 同一渠道多个日期节点在发货建议 Excel 中只输出渠道合计，批次日期保留在 `decisions.scenario_nodes`。
